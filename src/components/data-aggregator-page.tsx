...
export default function ExcelDataAggregatorPage({ onProcessingChange, onFileStateChange, onDirtyStateChange }: ExcelDataAggregatorPageProps) {
  ...
  const handleSaveSettings = () => {
      ...
      .then(() => {
          ...
          toast({ title: t('aggregator.toast.settingsSavedSuccessTitle') as string, description: t('aggregator.toast.settingsSavedSuccessDesc') as string });
      })
      .catch(err => {
          console.error("Failed to save settings:", err);
          toast({ title: t('aggregator.toast.settingsSavedErrorTitle') as string, description: t('aggregator.toast.settingsSavedErrorDesc') as string, variant: 'destructive' });
      })
      .finally(() => setIsProcessing(false));
  };

  const handleResetSettings = () => {
      if (window.confirm(t('aggregator.confirmReset') as string)) {
        setSettings(defaultSettings);
      }
  };
  ...
  const handleRangeSelection = useCallback(() => {
    ...
    for (const part of parts) {
        ...
        if (part.includes('-')) {
            ...
            if (!isNaN(index) && index <= sheetNames.length) {
  ...
  if (!settings) {
      return (
          <Card className="w-full max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-6xl shadow-xl relative flex items-center justify-center p-8">
              ...
              <p className="ml-4 text-lg">{[t('aggregator.loadingSettings')].flat().join(' ')}</p>
          </Card>
      )
  }
        ...
        <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
                <Button onClick={handleSaveSettings} disabled={!isDirty || isProcessing}>
                    ...
                    {[t('aggregator.saveSettings')].flat().join(' ')}
                ...
                <Button onClick={handleResetSettings} variant="outline" disabled={isProcessing}>
                    ...
                    {[t('aggregator.resetToDefaults')].flat().join(' ')}
                </Button>
            </div>
        </div>
